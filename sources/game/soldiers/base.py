...
class SoldierModel(GameObjectModel):
    ...
    def __init__(self, x: int, y: int, color: str, level: int = 1) -> None:
        ...
        if self.color == C.BLUE:
            self.boundaries = (1, C.HORIZONTAL_FIELD_TILE_COUNT - 2, 1, C.VERTICAL_TILE_COUNT - 2)
        else:
            self.boundaries = (0, C.HORIZONTAL_FIELD_TILE_COUNT - 1, 0, C.VERTICAL_TILE_COUNT - 1)
